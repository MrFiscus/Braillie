...
from the React app. Options: `--camera N|URL|video.mp4`, `--auto-page W H`, `--calib calibration.json`, `--mode read`,
`--mode word-quiz --words cap cat`, `--questions N`, `--no-mic`, `--port`. Standard library only, no extra install.
...
| GET | `/api/cells` | the A-Z sheet cells in page mm: `{cells:[{x,y,w,h,letter,dots,row,col}]}` |
...
```json
{
  "config":  {"mode": "letters", "commands": ["start quiz", "repeat", "hint", "found it", "next", "stop"]},
  ...
  "finger":  {"page_mm": [62.5, 51.0], "cell": {"letter": "h", "dots": [1, 2, 5], "row": 0, "col": 7}},
...
`finger` is whatever was last posted to `/api/finger` (there is no fingertip tracker yet). `finger.cell` is only filled in
letters mode.
