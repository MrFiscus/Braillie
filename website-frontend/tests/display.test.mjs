...
test('"match my device" becomes a real theme: more contrast wins (in the light or dark it prefers), then dark, then light', () => {
  ...
  assert.equal(resolveTheme('auto', false, true), 'contrast-light')
})

test('a chosen theme is never overruled by the device', () => {
  for (const s of ['light', 'dark', 'contrast-light', 'contrast']) assert.equal(resolveTheme(s, s !== 'dark', s !== 'light'), s)
})
...
test('saved preferences are read back, and nonsense falls back to the defaults', () => {
  ...
  assert.deepEqual(parsePrefs('{"scheme":"contrast"}'), { scheme: 'contrast', size: '1' }, 'saved earlier, still valid')
  assert.deepEqual(parsePrefs('{"scheme":"contrast-light","size":"4"}'), { scheme: 'contrast-light', size: '4' })
